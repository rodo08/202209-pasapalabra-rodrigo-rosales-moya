    ...
    let launchQuestions = () => {
        for(let i=0; i<questionsCopy.length; i++) {
            if(questionsCopy[i].status === 0) {
                document.getElementById('singleQuestionLetter').innerHTML = questionsCopy[i].question;
...
//////////////////////////////////FUNCION PARA RESPONDER Y BOTONES DE RESPUESTA Y PASAR
    ...
    let resolveQuestions = () => {
        ...
        if(typedAnswer === tempAnswer) {
            ...
            document.getElementById(letterId.toString()).style.backgroundColor = '#9ce4c5';
            counterOk++;
            ...
            document.getElementById('singleQuestionLetter').innerHTML = 'Incorrecto!';
            document.getElementById(letterId.toString()).style.backgroundColor = '#e4a79c';
            counterMiss++;
            userAnswerInput.value = '';
            questionsCopy.splice(0, 1);
            //console.log('wrong');
        ...
        document.getElementById('singleQuestionLetter').innerHTML = 'Pasapalabra!';
        document.getElementById(letterId.toString()).style.backgroundColor = '#e4e29c';
...
let launchRemainingQuestions = () =>{
    for(let i=0; i<[pasapalabraArr].length; i++) {
        if(pasapalabraArr[i].status === 0) {
            document.getElementById('singleQuestionLetter').innerHTML = pasapalabraArr[i].question;
...
let resolverRemainingQuestions = () => {
    userAnswerInput = document.getElementById('user-answer-input');
    userAnswerInput.addEventListener('input', ()=>{
    typedAnswer = userAnswerInput.value;
    typedAnswer = typedAnswer.toLowerCase();
...
//////////////////////////////////BOTON "SIGUIENTE"    
    ...
    nextQuestion.addEventListener('click', ()=> {
        if(questionsCopy.length > 0) {
            ...
            document.getElementById('counters').outerHTML = `<p id="counters" class="counter">Respondiste ${counterOk} respuestas correctas :)</p>`;
            document.getElementById('singleQuestionLetter').innerHTML = 'Respondiste todas las preguntas! :D';
...
//////////////////////////////////TEMPORIZADOR   
        ...
        const timer = setInterval(()=>{
            seconds--;
            document.getElementById('seg').outerHTML = `<p id="seg" class="seg">${seconds}</p>`;
            if(seconds == 0) {
                clearInterval(timer);
            }

            if(seconds == 0) {
                document.getElementById('singleQuestionLetter').innerHTML = 'Se acabo el tiempo! :(';
                document.getElementById('counters').outerHTML = `<p id="counters" class="counter">Respondiste ${counterOk} respuestas correctas :)</p>`;
                pasapalabra.disabled = true;
                answerButton.disabled = true;
